...
export function CautionButton({ label, size = "px-6 w-50 py-2 mx-2.5", className, buttonType = "button", isCancel = false, action }) {
    ...
    return (
        <button
            type={buttonType}
            onClick={action}
            className={className || `cursor-pointer rounded-sm mx-2.5 text-white ${variantColor} ${size}`}
        >
            {label}
        </button>
    )
...
export function BackButton({ label, className = "", buttonType = "button", action }) {
    return (
        <button
            type={buttonType}
            onClick={action}
            className={className || "px-6 w-25 py-1 cursor-pointer rounded-md bg-white text-black border border-black"}
